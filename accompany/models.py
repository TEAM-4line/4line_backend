...
class Accompany(models.Model):
    ...
    def __str__(self):
        return f"{self.user.username} - {self.age} - {self.travel_area} - {self.travel_period}"
...
class Comment(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.id:
            # 기존 댓글 수를 세서 익명 이름을 '익명' + (총 댓글 수 + 1) 형태로 지정
            existing_comments = Comment.objects.filter(post=self.post).count() + 1
